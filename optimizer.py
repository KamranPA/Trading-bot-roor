...
def optimize():
    ...
    max_test_profit = -1 # معیارِ موفقیت ما، سود در دیتایِ ندیده (Test) است
    ...
    for tp, sl in itertools.product([0.01, 0.03, 0.05], [0.01, 0.02]):
        ...
        for s in symbols:
            ...
            if os.path.exists(path) and os.path.getsize(path) > 200:
                df = pd.read_csv(path)
                # تقسیم دیتا: ۸۰٪ اول برای آموزش، ۲۰٪ آخر برای تست
                split_idx = int(len(df) * 0.8)
                train_df = df.iloc[:split_idx]
                test_df = df.iloc[split_idx:]
                
                # بهینه‌سازی روی train_df انجام می‌شود ولی معیار انتخاب، سود در test_df است
    ...
    print(f"✨ بهینه‌سازی با متدِ Out-of-Sample کامل شد: {best_config}")
